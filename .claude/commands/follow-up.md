...
description: Restore prior session work context, or analyze the structure of a target file/folder
---
...
1. Treat `$ARGUMENTS` (trimmed) as a candidate file or folder path.
2. Resolve it against the current working directory. Check existence (file OR folder).
3. **If the path exists:** skip Step 1 and Step B-1 entirely. Set the target to this path and jump directly to **B-2 (Recursive Dependency Analysis)**.
4. **If the path does not exist** (or the argument is clearly not a path — contains "?", multiple spaces of natural-language tokens, etc.): ignore the argument and proceed to Step 1 as usual. Briefly mention in the first user-facing line that the argument was not recognized as a valid path.
...
AskUserQuestion({
  questions: [{
    ...
    options: [
      ...
      { label: "구조 파악", description: "특정 파일/폴더의 의존성 구조를 재귀적으로 분석합니다" }
    ],
    multiSelect: false
  }]
})
...
#### B-1. Target Path Input

`AskUserQuestion` requires `options.minItems: 2`. Build the options list dynamically from git changes so the user can one-click recently-touched files, with Other as the free-input fallback.

1. Read the git status / diff already in the Context section. Extract up to **3** modified or untracked file paths as suggested options.
2. Always append a fixed `"경로 직접 입력"` option as the last item — this guarantees the array has ≥ 2 items even when there are zero git changes.

**Case A — git changes exist (1 or more files):**

```
AskUserQuestion({
  questions: [{
    question: "분석할 파일 또는 폴더 경로를 선택해주세요. (Other를 눌러 직접 입력 가능)",
    header: "Target Path",
    options: [
      ...
      { label: "경로 직접 입력", description: "Other를 선택하여 분석할 파일/폴더 경로를 직접 입력합니다" }
    ],
    multiSelect: false
  }]
})
...
- Include only as many git-change options as actually exist (max 3). Total options including the fixed "경로 직접 입력" must stay ≤ 4.
- If the user selects a git-changed option, use that path directly as the target.
- If the user selects Other (free input), use the typed path.

**Case B — no git changes at all:**

```
AskUserQuestion({
  questions: [{
    question: "분석할 파일 또는 폴더 경로를 입력해주세요. (Other를 선택하여 직접 입력)",
    header: "Target Path",
    options: [
      { label: "경로 직접 입력", description: "Other를 선택하여 분석할 파일/폴더 경로를 직접 입력합니다" },
...
- If it does not exist, output "경로를 찾을 수 없습니다: `<path>`" and stop.
- If it is a file, analyze that single file. If it is a folder, treat every source file inside as an entry point.
